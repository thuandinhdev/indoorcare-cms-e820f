...
eleventyConfig.addShortcode("youtubeinfoblock", function (raw) {
  const firstSepIndex = raw.indexOf("|");
  const youtube = raw.substring(0, firstSepIndex).trim();
  const infoStr = raw.substring(firstSepIndex + 1).trim();

  const infos = infoStr
    ? infoStr.split("@@").map(info => ({ info: info.trim() }))
    : [];

  const youtubeEmbed = youtube.replace("watch?v=", "embed/");
            ...
            <div class="col-lg-6 col-md-6 col-12 mt-4 pt-2">
              <div class="section-title ml-lg-5">
                <ul class="font-weight-bold">
                  ${infos.map(i => `
                    <li class="mb-0"><span class="text-primary h5 mr-2"><i class="uim uim-check-circle"></i></span>${i.info}
                    </li>
                  `).join("")}
                </ul>
              </div>
